...
class Schedule_model
{
  ...
  public function getUserTeamMember()
  {
    $this->db->query("SELECT * FROM user WHERE role IN ('Team Member', 'Administrator')");
    return $this->db->resultSet();
  }
  ...
  public function getDetailProject($id)
  {
    ...
    $this->db->query('SELECT * FROM user WHERE nik = :name');
...
WHERE st.id_project = :id
GROUP BY st.id_step;
');
    $this->db->bind('id', substr($id, 0, 11));
    return $this->db->resultSet();
  }
